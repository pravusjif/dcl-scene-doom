...
import { AssetLoad, LoadingState, GltfContainer, Transform } from '@dcl/sdk/ecs'
...
```typescript
// Hide: move underground
Transform.getMutable(entity).position = Vector3.create(0, -100, 0)
// Show: move to target position
Transform.getMutable(entity).position = targetPosition
```
